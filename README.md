...
#### Stress tests with k6
 ...
 the [`README.md`](https://github.com/anton-k/reader-pattern-mig-app/tree/main/test-stress)
...
* with Reader-approach we can decopule implementation from the mig-server details
...
The [Reader pattern](https://github.com/anton-k/reader-pattern-mig-app) is one
...
newtype App env a = App (ReaderT env IO a)
  deriving newtype
    ...
    , MonadThrow
    , MonadCatch
    , HasServer -- we need this instance to convert to mig IO-server
    )
...
server =
  ...
  where
    ...
    handleSave (Body req) = Send $ fmap ok $ withApp (.save) $ Save.handle req

    handleGetById :: Capture "message-id" MessageId -> Get (App Env) (RespOr ApiError Message)
    handleGetById (Capture messageId) = Send $ withApp (.getMessage) $
      either (bad status400) ok <$> GetMessage.handle messageId

    handleGetByTag :: Capture "tag" Tag -> Get (App Env) (Resp [Message])
    handleGetByTag (Capture tag) = Send $ fmap ok $ withApp (.listTag) $ ListTag.handle tag

    handleToggleLogs :: Post (App Env) (Resp ())
    handleToggleLogs = Send $ fmap ok $ withApp (.toggleLogs) ToggleLog.handle
```

Here we use function `withApp` that is analog to `withReaderT` to zoom inside a part of `Env`
as environment for the reader. Because all our methods are defined in terms of specific context
which contains only info that specific route needs:

```haskell
withApp :: (envA -> envB) -> App envB a -> App envA a
...
web-apps with [mig](https://github.com/anton-k/mig) 
that are easy to change and keep development with the wave.

Let's mention the points:

* we can organise application with ReaderT, keeping our mtl-stack shallow and simple.

* with Reader-approach we can decopule implementation from the mig details
